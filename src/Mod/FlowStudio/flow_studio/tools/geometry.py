"""Canonical geometry checking helpers for FlowStudio tools."""
...
def run_leak_tracking(source_ref, target_ref, document=None, tolerance=1e-5):
	"""Estimate whether a leak path may exist between two selected faces."""
	document = document or FreeCAD.ActiveDocument
	objects = list(iter_geometry_objects(document=document)) if document is not None else []
	source_obj = source_ref[0] if source_ref else None
	target_obj = target_ref[0] if target_ref else None
	if source_obj is None or target_obj is None:
		return {"status": "INVALID_SELECTION", "messages": ["Select two faces to run leak tracking."]}

	graph = _build_connectivity_graph(objects, tolerance=tolerance)
	connected = _connected_component(graph, getattr(source_obj, "Name", ""))
	if getattr(target_obj, "Name", "") not in connected:
		return {
			"status": "NO_CONNECTION",
			"messages": [
				f"No direct leak path detected between {describe_face_ref(source_ref)} and {describe_face_ref(target_ref)}."
			],
		}

	open_objects = []
	for object_name in connected:
		obj = _lookup_object(objects, object_name)
		if obj is None:
			continue
		topology = analyze_shape_topology(getattr(obj, "Shape", None))
		if topology.free_edge_count or topology.non_manifold_edge_count:
			open_objects.append(getattr(obj, "Label", object_name))

	if open_objects:
		return {
			"status": "POSSIBLE_LEAK",
			"messages": [
				f"Possible leak path found between {describe_face_ref(source_ref)} and {describe_face_ref(target_ref)}.",
				f"Connected geometry still contains unresolved openings: {', '.join(sorted(open_objects))}.",
			],
		}

	return {
		"status": "NO_LEAK_FOUND",
		"messages": [
			f"Connected closed geometry found between {describe_face_ref(source_ref)} and {describe_face_ref(target_ref)}.",
		],
	}